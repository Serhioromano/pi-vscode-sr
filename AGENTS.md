...
Pi (terminal)                               VS Code (diff UI)
    ...
    │  └─ ❌ нет: мгновенный approve                │
    │     → прямой write (без ревью)                │
    │     → ⚠️ предупреждение (один раз)            │
    │  └─ ✅ да:                                    │
...
- Файл существует **и** timestamp свежий (≤ 30 секунд) → VS Code жив → ревью через diff editor (+ TUI fallback)
- Иначе → **прямой write без ревью** (агент работает в стандартном режиме)

При первом обнаружении отсутствия VS Code показывается одноразовое предупреждение:
`⚠️ VS Code not detected — working without diff review. All file changes will be applied directly.`
...
**Approve All** работает только в рамках одного промпта (`before_agent_start` сбрасывает).
